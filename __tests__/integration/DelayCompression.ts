...
describe("RFC 8308 delay-compression integration", () => {
    test("activates each direction at its trigger and exchanges compressed traffic", async () => {
        ...
        try {
            ...
            expect(client.serverCompressionAlgorithm?.alg_name).toBe("zlib")
            expect(peer?.clientCompressionAlgorithm?.alg_name).toBe("zlib")
            expect(peer?.serverCompressionAlgorithm?.alg_name).toBe("zlib")

            await client.rekey()
            expect(client.clientCompressionAlgorithm?.alg_name).toBe("none")
            expect(client.serverCompressionAlgorithm?.alg_name).toBe("none")
            expect(await client.globalRequest("compressed@example.com", payload)).toEqual(payload)
        } finally {
            client.destroy()
    ...
    test.each([
        ...
        async (clientDelayCompression, serverDelayCompression) => {
            ...
            try {
                ...
                expect(client.clientCompressionAlgorithm?.alg_name).toBe("none")
                expect(client.serverCompressionAlgorithm?.alg_name).toBe("none")
                expect(peer?.clientCompressionAlgorithm?.alg_name).toBe("none")
                expect(peer?.serverCompressionAlgorithm?.alg_name).toBe("none")
            } finally {
                client.destroy()
                for (const connection of server.clients) connection.terminate()
                await server.close()
