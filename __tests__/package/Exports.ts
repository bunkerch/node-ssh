...
describe("package exports", () => {
    test("exposes the supported source API without executing a demo", () => {
        ...
        expect(SSHAgentProtocolClient.prototype.close).toBeFunction()
        expect(SSHAgentProtocolClient.prototype[Symbol.asyncDispose]).toBeFunction()
        ...
        expect(ProtocolVersionExchange.defaultValue.protocol_software).toBe("modernssh_1.0.1")
        expect(SSHFPRecord).toBeFunction()
        expect(SSHFPAlgorithm.Ed25519).toBe(4)
        expect(SSHFPFingerprintType.SHA256).toBe(2)
        expect(verifySSHFP).toBeFunction()
    })
    ...
    test("compiled entry point provides the same side-effect-free API", async () => {
        ...
        expect(entry.ProtocolVersionExchange.defaultValue.protocol_software).toBe("modernssh_1.0.1")
        ...
        expect(entry.SSHAgentProtocolClient.prototype.close).toBeFunction()
        expect(entry.SSHAgentProtocolClient.prototype[Symbol.asyncDispose]).toBeFunction()
    ...
    test("compiled declarations expose Promise-only completion APIs", async () => {
        ...
        const sessionChannel = await readFile("dist/channels/SessionChannel.d.ts", "utf8")
        ...
        expect(agentProtocol).toContain("close(): Promise<void>")
        expect(agentProtocol).toContain("[Symbol.asyncDispose](): Promise<void>")
        ...
        try {
            ...
            const { stdout, stderr } = await execFileAsync(
                ...
                [
                    ...
                    if (ProtocolVersionExchange.defaultValue.protocol_software !== "modernssh_1.0.1") process.exit(116)
                    ...
                    const packedSSHFP = SSHFPRecord.fromPublicKey(publicKey, SSHFPFingerprintType.SHA256)
                    if (SSHFPRecord.parseText(packedSSHFP.toString()).toString() !== packedSSHFP.toString()) process.exit(107)
                    if (verifySSHFP(publicKey, [packedSSHFP]) !== "match") process.exit(108)
