...
closing the otherwise healthy connection. After an accepted end-of-write, window-change, or signal
reaches its synchronous observation event, a thrown listener error follows the owning connection's
error path without generating a forbidden reply. Malformed setup requests that ask for a reply
receive channel failure.
