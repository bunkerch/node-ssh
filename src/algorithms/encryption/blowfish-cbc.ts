...
// Blowfish initializes its P-array and S-boxes with the fractional hexadecimal digits of pi.
const INITIAL_STATE_HEX = [
...
const INITIAL_STATE = (() => {
    const words = new Uint32Array(P_WORDS + S_WORDS)
    for (let index = 0; index < words.length; index++) {
        words[index] = Number.parseInt(INITIAL_STATE_HEX.slice(index * 8, index * 8 + 8), 16)
    }
    return words
})()
...
export default class BlowfishCBC implements EncryptionAlgorithm {
    static alg_name = "blowfish-cbc"
    ...
    constructor(key: Buffer, iv: Buffer) {
        if (key.length !== BlowfishCBC.key_length) {
            throw new Error(`SSH Blowfish key must be ${BlowfishCBC.key_length} bytes`)
        }
        if (iv.length !== BlowfishCBC.iv_length) {
            throw new Error(`SSH Blowfish IV must be ${BlowfishCBC.iv_length} bytes`)
        }
    ...
    encrypt(plaintext: Buffer): Buffer {
        ...
        for (let offset = 0; offset < plaintext.length; offset += BLOCK_SIZE) {
            ...
            const [encryptedLeft, encryptedRight] = this.encryptWords(left, right)
    ...
    decrypt(ciphertext: Buffer): Buffer {
        ...
        for (let offset = 0; offset < ciphertext.length; offset += BLOCK_SIZE) {
            ...
            const [left, right] = this.decryptWords(encryptedLeft, encryptedRight)
    ...
    private expandKey(key: Buffer): void {
        let keyOffset = 0
        for (let index = 0; index < P_WORDS; index++) {
            let word = 0
            for (let byte = 0; byte < 4; byte++) {
                word = ((word << 8) | key[keyOffset]) >>> 0
                keyOffset = (keyOffset + 1) % key.length
            }
            this.#state[index] = (this.#state[index] ^ word) >>> 0
        }

        let left = 0
        let right = 0
        for (let index = 0; index < this.#state.length; index += 2) {
            ;[left, right] = this.encryptWords(left, right)
            this.#state[index] = left
            this.#state[index + 1] = right
        }
    }

    private encryptWords(left: number, right: number): readonly [number, number] {
        let encryptedLeft = left >>> 0
        let encryptedRight = right >>> 0
        for (let round = 0; round < 16; round++) {
            encryptedLeft = (encryptedLeft ^ this.#state[round]) >>> 0
            encryptedRight = (encryptedRight ^ this.f(encryptedLeft)) >>> 0
            ;[encryptedLeft, encryptedRight] = [encryptedRight, encryptedLeft]
        }
        ;[encryptedLeft, encryptedRight] = [encryptedRight, encryptedLeft]
        encryptedRight = (encryptedRight ^ this.#state[16]) >>> 0
        encryptedLeft = (encryptedLeft ^ this.#state[17]) >>> 0
        return [encryptedLeft, encryptedRight]
    }

    private decryptWords(left: number, right: number): readonly [number, number] {
        let decryptedLeft = left >>> 0
        let decryptedRight = right >>> 0
        for (let round = 17; round > 1; round--) {
            decryptedLeft = (decryptedLeft ^ this.#state[round]) >>> 0
            decryptedRight = (decryptedRight ^ this.f(decryptedLeft)) >>> 0
            ;[decryptedLeft, decryptedRight] = [decryptedRight, decryptedLeft]
        }
        ;[decryptedLeft, decryptedRight] = [decryptedRight, decryptedLeft]
        decryptedRight = (decryptedRight ^ this.#state[1]) >>> 0
        decryptedLeft = (decryptedLeft ^ this.#state[0]) >>> 0
        return [decryptedLeft, decryptedRight]
    }

    private f(value: number): number {
        const s0 = this.#state[P_WORDS + (value >>> 24)]
        const s1 = this.#state[P_WORDS + 256 + ((value >>> 16) & 0xff)]
        const s2 = this.#state[P_WORDS + 512 + ((value >>> 8) & 0xff)]
        const s3 = this.#state[P_WORDS + 768 + (value & 0xff)]
        return add32((add32(s0, s1) ^ s2) >>> 0, s3)
    }
